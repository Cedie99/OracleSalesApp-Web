...
Two related things web still owes, both from the vault's `Web-Migration-Backfill-Action-2026-07-26.md`:

- **Seventeen backfill files — 025–040 and 042.** All already live, no SQL to re-run; web's repo just has no file saying they exist. Until then a rebuild from this repo cannot reproduce production. The `Web-Migration-Backfill-Action-2026-07-26.md` note names only 025/026/027 because it was written before Batch 2 and Batch 3 landed — the debt has grown to seventeen since. Two cautions when it does get done: migration 034 must be captured as **corrected**, not as first applied (it went in with a wrong identity column and a guessed policy name, both fixed the same day), and none of the seventeen are recorded in the remote `schema_migrations`, so they need `supabase migration repair --status applied` rather than a plain `db push`.
- Web's `023_add_delivery_role.sql` is the documented precedent for why this matters: it rewrote `profiles_role_check` without knowing the mobile side had added a value to it, silently dropping that value and breaking a live account. Exactly the collision 043/044 just avoided.
