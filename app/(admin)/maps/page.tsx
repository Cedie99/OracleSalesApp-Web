...
import type { MapPin, FocusTarget, HighlightMarker } from '@/components/maps/meetings-map'
...
const MeetingsMap = dynamic(() => import('@/components/maps/meetings-map'), {
  ssr: false,
  loading: () => (
...
export default function MapsPage() {
  ...
  const onlineOnlyCount = visited.length - mappedCount
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <Header
        title="Meetings Map"
        subtitle={`${rangeLabel} · ${visited.length} ${visited.length === 1 ? 'client' : 'clients'} visited · ${mappedCount} mapped · ${onlineOnlyCount} online`}
      />
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 relative min-h-0">
          ...
          {/* Explains a bare map so it doesn't read as a bug. */}
          {mappedCount === 0 && (
                ...
                <div className="text-xs text-muted-foreground leading-relaxed">
                  {listMode === 'unvisited' ? (
                    ...
                    <>
                      ...
                      These are online meetings or predate GPS capture, so they can&apos;t be pinned. They&apos;re listed on the left.
                    </>
                  )}
                </div>
              </CardContent>
          ...
          {listMode === 'visited' && (
            <Card className="absolute top-4 left-4 w-48 bg-card/95 border-border backdrop-blur-sm z-[1000] pt-0 gap-0">
          ...
          {/* ---- Detail overlay ---------------------------------------------- */}
          {selected && (
            <div className="absolute top-4 right-4 bottom-4 w-80 z-[1000] flex flex-col rounded-xl border border-border bg-card/95 backdrop-blur-sm shadow-lg overflow-hidden">
