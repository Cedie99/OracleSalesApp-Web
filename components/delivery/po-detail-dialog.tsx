  ...
  return (
    <>
      <Dialog open={!!po} onOpenChange={open => !open && onOpenChange(false)}>
        {/* Wide + height-capped: the record can run long (COD installments,
            proofs, map), so it spreads across two columns and scrolls inside
            rather than growing off the top and bottom of the screen. */}
  ...
  return (
    <>
      ...
      <div className="space-y-3 overflow-y-auto -mr-2 pr-2 text-sm">
        ...
        {/* Backloaded notice is full width — it's the headline outcome of the
            whole run, not a detail to tuck into a column. */}
        {po.status === 'failed' && (
        ...
        {/* Two columns on a wide screen: the run + money (left) beside the field
            evidence (right). Stacks to one column when it can't fit. */}
        <div className="grid gap-3 sm:grid-cols-2 sm:items-start">
          {/* Left — who listed it, the run, and the COD trail. */}
          <div className="space-y-3">
            ...
            {/* COD installment history — one row per handover, each with its own proof
                ...
                grid only shows the latest one. */}
