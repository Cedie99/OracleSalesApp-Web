...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // BizLink card: borderless on a tinted canvas, large radius, whisper
        // shadow instead of a ring — separation comes from bg contrast.
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl bg-card py-(--card-spacing) text-sm text-card-foreground shadow-[0_1px_2px_rgba(18,39,28,0.05)] [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        className
      )}
      {...props}
    />
